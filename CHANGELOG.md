...
### Changed
- **Karaoke caption colour** — words not yet spoken now show as a **dimmed version
  of each speaker's own colour** (which brightens to the full colour as each word is
  spoken), instead of a fixed red. Each speaker now reads as one coherent colour.
